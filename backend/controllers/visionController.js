...
// Multer setup to parse incoming file
...
exports.scanCheck = async (req, res) => {
  try {
    ...
    // Upload to AWS S3
    const filename = `checks/${Date.now()}-${uuidv4()}-${req.file.originalname}`;
    const s3Params = {
      Bucket: process.env.AWS_S3_BUCKET,
      Key: filename,
      Body: req.file.buffer,
...
exports.scanLicense = async (req, res) => {
  try {
    ...
    const filename = `licenses/${Date.now()}-${uuidv4()}-${req.file.originalname}`;
